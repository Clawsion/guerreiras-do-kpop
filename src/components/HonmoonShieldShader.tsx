...
/* ═══════════════════════════════════════════════════════════════════
   HONMOON SHIELD — Animated Shader Background
   
   Tunnel/mandala animation that wraps around the existing
   Honmoon shield arcs, strands, and nodes.
   Wider, more expanded — an upgrade to the existing static CSS.
   
   Dark mode: Neon Purple / Pink / Blue
   Light mode: Golden / Iridescent Rainbow pastels
...
void main() {
  ...
  /* Main rings — wider for the shield */
  ...
  /* Wider mandala rings — fill the whole section */
  ...
  /* Vignette — wider for the shield section */
  float vignette = 1.0 - smoothstep(0.3, 1.8, length(uv * 0.6));

  vec3 finalColor = vec3(0.0);
  finalColor += color * brightness * vignette;
  finalColor += centerColor * centerTotal * vignette * 1.2;
  finalColor += color * spiralPattern * vignette * 0.4;
  finalColor += color * petalPattern * vignette * 0.3;

  /* Depth shimmer */
  finalColor *= sin(angle * 6.0 + t * 0.5 + dist * 4.0) * 0.05 + 1.0;

  /* Dark mode edge darkening */
  finalColor *= mix(1.0, smoothstep(1.8, 0.3, dist), u_darkMode * 0.3);

  /* Make it more transparent/ghostly so CSS elements show through */
  finalColor *= 0.55;
...
export default function HonmoonShieldShader() {
  ...
  const initGL = useCallback(() => {
    ...
    if (!gl.getShaderParameter(vs, gl.COMPILE_STATUS)) {
      console.error("HM VS:", gl.getShaderInfoLog(vs));
      return false;
    }
    ...
    if (!gl.getShaderParameter(fs, gl.COMPILE_STATUS)) {
      console.error("HM FS:", gl.getShaderInfoLog(fs));
      return false;
    }
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error("HM Link:", gl.getProgramInfoLog(program));
      return false;
    }
  ...
  return (
    <canvas
      ref={canvasRef}
      className="honmoon-shield-canvas"
    />
  );
}

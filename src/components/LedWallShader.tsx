...
/* ═══════════════════════════════════════════════════════════════════
   LED WALL — Honmoon Mandala Shader (Performance-Smart)
   
   Original animation preserved — safe optimizations added:
   - IntersectionObserver: pauses GPU when section is off-screen
   - Tab-hidden detection: stops all GPU work when tab is hidden
   - Cached uniform locations: avoids per-frame getUniformLocation calls
   - Adaptive DPR: caps devicePixelRatio on low-end devices
   - Mobile detection: simpler rendering path for smartphones
...
export default function LedWallShader({ active }: { active: boolean }) {
  ...
  const initGL = useCallback(() => {
    ...
    const gl = canvas.getContext("webgl", {
      antialias: !IS_MOBILE,  // Skip AA on mobile for better performance
      alpha: false,
  ...
  /* ── Resize canvas to container — adaptive DPR ── */
